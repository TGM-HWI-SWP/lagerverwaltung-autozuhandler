...
class PartService:
    def __init__(
        self,
        part_repository: PartRepositoryPort,
    ...
    def create_part(
        ...
        if self.movement_service:
            self.movement_service.log(
                entity_type="Teil",
                entity_id=part["id"],
                action="Erstellt",
                description=f"Teil {part['name']} angelegt",
                quantity_delta=int(part["stock"]),
                reference=part["status"],
            )
    ...
    def update_part(
        ...
        old_stock = int(existing.get("stock", 0))
        old_status = safe_str(existing.get("status"))
        ...
        if self.movement_service:
            new_stock = int(part["stock"])
            delta = new_stock - old_stock
            status_text = ""
            if old_status != part["status"]:
                status_text = f" (Status: {old_status} → {part['status']})"

            self.movement_service.log(
                entity_type="Teil",
                entity_id=part["id"],
                action="Aktualisiert",
                description=f"Teil {part['name']} aktualisiert{status_text}",
    ...
    def delete_part(self, selected_id: str) -> ServiceResult:
        ...
        if self.movement_service:
            self.movement_service.log(
                ...
                action="Gelöscht",
                description=f"Teil {deleted_text} gelöscht",
                quantity_delta=-int(existing.get("stock", 0)),
                reference=existing.get("status", ""),
            )
