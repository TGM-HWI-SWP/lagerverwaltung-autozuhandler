    ...
    def create_customer(
        ...
        if self.movement_service:
            self.movement_service.log(
                ...
                action="Erstellt",
                description=f"Kunde {customer['name']} angelegt",
                quantity_delta=0,
                reference=customer.get("email", ""),
            )
    ...
    def update_customer(
        ...
        if self.movement_service:
            self.movement_service.log(
                ...
                entity_id=customer["id"],
                action="Aktualisiert",
                description=f"Kunde {customer['name']} aktualisiert",
                quantity_delta=0,
                reference=customer.get("email", ""),
            )
    ...
    def delete_customer(self, selected_id: str) -> ServiceResult:
        ...
        if self.movement_service:
            self.movement_service.log(
                ...
                action="Gelöscht",
                description=f"Kunde {existing['name']} gelöscht",
                quantity_delta=0,
