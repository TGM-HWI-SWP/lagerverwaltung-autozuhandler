...
def get_movement_action_chart(data: list[dict] | None = None):
    source = data or []
    counts = {"Erstellt": 0, "Aktualisiert": 0, "Gelöscht": 0}

    for entry in source:
        action = safe_str(entry.get("action"))
        if action in counts:
            counts[action] += 1

    if sum(counts.values()) == 0:
        return get_empty_figure("Bewegungen nach Aktion")
    ...
    ax.set_title("Bewegungen nach Aktion", color="white", fontsize=14, pad=16)
    ax.tick_params(axis="x", colors="white")
    ax.tick_params(axis="y", colors="white")
    ax.spines["bottom"].set_color("#64748b")
    ax.spines["left"].set_color("#64748b")
